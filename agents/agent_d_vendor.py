...
def _price_variance_pct(
    ...
    return round(
        ((requested_price - catalogue_price) / catalogue_price) * 100, 2
    )
...
def run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # ── AuditLogger dhe MetricsTracker nga state ─────────────
    ...
    try:
        ...
        for line in extracted_pr.line_items:
            ...
            if vendor_record is None:
                ...
                findings.append(
                    _make_finding(
                        ...
                        description        = (
                            f"Requested vendor '{requested_vendor}' was not "
                            f"found in the approved vendor list."
                        ),
                        field_name         = (
                ...
                is_approved    = _to_bool(
                    vendor_record.get(
                        "is_approved", vendor_record.get("approved", False)
                    )
                )
                is_preferred = _to_bool(
                    vendor_record.get(
                        "is_preferred", vendor_record.get("preferred", False)
                    )
                )
            ...
            if catalogue_price is None:
                findings.append(
                    _make_finding(
                        ...
                        field_name         = (
                            f"line_items[{line.line_number - 1}].item_name"
                        ),
                        recommended_action = (
            ...
            line_match = VendorLineMatch(
                ...
                price_variance_pct = _price_variance_pct(
                    line.unit_price, catalogue_price
                ),
                status   = status,
                findings = findings,
            )

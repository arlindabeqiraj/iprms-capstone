...
def test_agent_c_bundle_004():
    """
    Bundle 004 — Emergency Sole-Source.
    Expected: AVAILABLE (budget sufficient), capex_required=True
    (8,500 >= 5,001 threshold), period_cap_ok=True (8,500 < 20,000).
    PR004: Dell Server x1 = $8,500 vs available $15,000.
    """
    ...
    # ── Print results ────────────────────────────────────────
    print(f"\n{'='*50}")
    print(f"RESULT — AGENT C (Bundle 004)")
    print(f"{'='*50}")
    print(f"Overall Status : {result.overall_status.value}")
    print(f"Total Requested: {result.total_requested:.2f}")
    print(f"Total Available: {result.total_available:.2f}")
    ...
    # Exact values
    assert result.total_requested == 8500, \
        f"❌ Expected 8500, got {result.total_requested}"
    print(f"✅ total_requested = 8500 (correct)")

    assert result.total_available == 15000, \
        f"❌ Expected 15000, got {result.total_available}"
    print(f"✅ total_available = 15000 (correct)")
    ...
    # No findings — budget is available
    assert all(len(lc.findings) == 0 for lc in result.line_checks), \
        f"❌ Expected no findings for AVAILABLE status"
    print(f"✅ no findings produced (correct)")

    # Period cap OK — 8,500 < 20,000
    assert all(lc.period_cap_ok is True for lc in result.line_checks), \
        f"❌ Expected period_cap_ok=True (8500 < 20000)"
    print(f"✅ period_cap_ok = True (correct — 8500 < 20000)")

    # Capex required — 8,500 >= 5,001 threshold
    assert all(lc.capex_required is True for lc in result.line_checks), \
        f"❌ Expected capex_required=True (8500 >= 5001)"
    print(f"✅ capex_required = True (correct — 8500 >= 5001 threshold)")

    print(f"\n🎉 ALL TESTS PASSED — Bundle 004 (Agent C)!")


def test_agent_c_bundle_005():
    """
    Bundle 005 — Budget Borderline.
    Expected: BORDERLINE (9,200 > 90% of 10,000 = 9,000),
    Severity.WARNING finding, period_cap_ok=True, capex_required=True.
    PR005: Dell Laptop x10 = $9,200 vs available $10,000.
    """
    ...
    print(f"\n{'='*50}")
    print(f"RESULT — AGENT C (Bundle 005)")
    print(f"{'='*50}")
    print(f"Overall Status : {result.overall_status.value}")
    print(f"Total Requested: {result.total_requested:.2f}")
    print(f"Total Available: {result.total_available:.2f}")
    ...
    # BORDERLINE — 9,200 > 90% of 10,000
    assert result.overall_status.value == "BORDERLINE", \
        f"❌ Expected BORDERLINE, got {result.overall_status.value}"
    print(f"✅ overall_status = BORDERLINE (correct — 9200 > 9000)")

    # requested < available but > 90%
    assert result.total_requested < result.total_available, \
        f"❌ Expected requested < available"
    print(f"✅ total_requested < total_available (correct)")

    assert result.total_requested > result.total_available * 0.90, \
        f"❌ Expected requested > 90% of available"
    print(f"✅ total_requested > 90% of total_available (correct)")

    # Exact values
    assert result.total_requested == 9200, \
        f"❌ Expected 9200, got {result.total_requested}"
    print(f"✅ total_requested = 9200 (correct)")
    ...
    # WARNING finding — not BLOCK
    assert len(result.line_checks[0].findings) == 1, \
        f"❌ Expected 1 finding"
    print(f"✅ finding exists (correct)")

    assert result.line_checks[0].findings[0].severity.value == "WARNING", \
        f"❌ Expected WARNING severity"
    print(f"✅ severity = WARNING (correct — borderline, not blocked)")

    # Period cap OK — 9,200 < 15,000
    assert result.line_checks[0].period_cap_ok is True, \
        f"❌ Expected period_cap_ok=True (9200 < 15000)"
    print(f"✅ period_cap_ok = True (correct — 9200 < 15000)")

    # Capex required — 9,200 >= 5,001 threshold
    assert result.line_checks[0].capex_required is True, \
        f"❌ Expected capex_required=True (9200 >= 5001)"
    print(f"✅ capex_required = True (correct — 9200 >= 5001)")

    print(f"\n🎉 ALL TESTS PASSED — Bundle 005 (Agent C)!")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60 + "\n")
    test_agent_c_bundle_004()
    print("\n" + "=" * 60 + "\n")
    test_agent_c_bundle_005()